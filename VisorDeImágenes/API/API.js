"use strict"
let imagenMostrada=document.getElementById("imagenGrande");
...
const imagenes=document.getElementsByClassName("imagenesGrid");
fetch(url)
.then((response) => {
...
.then((data) => {
  ...
  imagesData.map(function(imageData) {
    ...
    let image="";
    image += `<img src="${imageData.url}" class="imagenesGrid w-44 h-28 m-auto rounded-lg opacity-70 hover:opacity-100 border-solid border-gray-900 border-2" onclick="ampliarImagen(this)">`;     
...
//Con esta funcion se amplia la imagen sobre la que se da click
